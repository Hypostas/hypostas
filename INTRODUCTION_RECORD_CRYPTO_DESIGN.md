...
- **What "B vouches for D" is.** B **blind-issues** D a one-attribute BBS credential on a *commitment* to D's `introduced_dyad_id` — **committed-attribute / blind BBS issuance**, matching the spec's §4 `σ' = BlindSign_B(blind(T))` flow. D presents an **issuer-hiding, unlinkable proof-of-possession** — the BBS component of `BlindedVouch` — proving "*an attested introducer in this epoch* signed a credential on D" and hiding which one **unconditionally**.
- **Blindness (§4.1) — requires BLIND issuance, not ordinary issuance (Codex DESIGN-review P1).** Ordinary BBS issuance would let a curious/compromised B link its issuance session to the published record via its own log: presentation re-randomization only unlinks the *proof* from the raw signature, **not** the issuer's *knowledge of what it signed*. So the issuance MUST be blind / committed-attribute (above); blind issuance + the unlinkable presentation together give §4.1 signer-blindness + log-unlinkability. (B still *knows* it vouched for D — accepted per §6 — but its cryptographic log can't be weaponized to prove its introductions to a colluding chain observer.)
