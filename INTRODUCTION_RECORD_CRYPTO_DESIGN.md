...
|---|---|---|
| §4.1 Blindness | Both halves use blind issuance + D-side unlinkable presentation, so a chain observer can't link without B's cooperation. **Strict §4.1** (a compromised B can't reconstruct even its *own* introductions) holds only under §4.1 binding **variant (b)** (committed dyad + ZK bond-equality); **variant (a)** (visible bound dyad) satisfies only the weaker **§6** model (B knows its own vouches — the public `(dyad, epoch)` is matchable in B's log) | variant (b): strict; variant (a): §6 only |
