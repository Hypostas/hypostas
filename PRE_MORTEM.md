...
- **Amendment (Josh, 2026-07-10):** Aether *spec-depth* work unfrozen for the Fable-5 window (resolve its open threads at design level); the **build gate is unchanged** — Aether still enters no build queue until the founder dyad ships. R2's complexity budget governs build-surface, and spec-deepening during a peak-reasoning window was judged worth the (bounded) attention cost. Recorded in `AETHER_SPEC.md`'s status banner.
