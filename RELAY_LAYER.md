...
- **(a) Unauthenticated cover-drop (no new key, recommended for v1).** A padding cell is a random Outfox-shaped cell (correct fixed size + clear header) that authenticates *nowhere* at R+1 — it falls through R+1's existing **cover-drop** path (`on_data_outfox`'s final fall-through already silently drops a cell matching no forward/terminal/reverse leg) and is consumed, never forwarded. Delivers the §6 **threat property** in full (the observer can't distinguish padding from data; R+1 consumes it). Cost: R+1 does the normal peel-attempt work before dropping, and cannot *affirmatively* tell padding from a corrupt/attack cell (so no distinct padding accounting) — acceptable, since flow-correlation resistance doesn't need affirmative recognition. **No protocol change; buildable now** as an emit-rate driver (reuse the `CoverTrafficScheduler` + `BandwidthBudget` step-down, COVER_TRAFFIC §5) over the existing forward sink.
...
**Recommendation:** ship **(a)** for HYP-321 v1 (the threat property with zero protocol change), and track **(b)** as a follow-up optimization. Until this is decided, HYP-321 is **not** a clean protocol-core cell primitive — the `CMD_DROP` command + a per-hop authenticated marker (the obvious-looking change) is exactly the part that needs the absent link key. *(The Linear issue for this finding is pending the MCP connector reconnect — see the build ledger.)*
