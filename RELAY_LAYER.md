...
- **(a) Unauthenticated cover-drop (no new key, recommended for v1 — IN PROGRESS).** A padding cell is a random Outfox-shaped cell (correct fixed size + clear header) with a **random `circuit_id`** that authenticates *nowhere* at R+1 — it falls through R+1's existing **cover-drop** path (`on_data_outfox`'s final fall-through already silently drops a cell matching no forward/terminal/reverse leg) and is consumed, never delivered. Delivers the §6 **threat property** in full (the observer can't distinguish padding from data; R+1 consumes it). This is exactly the model of the per-dyad EDGE cover cell (`sealed_envelope::cover_cell` — "the random `circuit_id` drops it at the receiver").
  ...
  - **No protocol change. BUILT:** the cell primitive `protocol_core::payload_onion::seal_outfox_padding_cell(size, circuit_id)` (random-id, indistinguishable from a forwarded DATA cell; an integration test proves a padding cell fed to a live receiver drops silently — dyados PR #443), and the cadence decision core `vita_carriers::cover_traffic::relay_padding::RelayPaddingScheduler` (off-by-default; enabled iff opt-in AND **unmetered** AND cover-class ≥ Standard; `on_slot(now, &shared_carrier_budget)` emits one slot per the carrier energy-class rate, yielding to budget pressure first — reuses `BandwidthBudget` + `EnergyClass` + `carrier_max_cover_class`; dyados PR #447). **Remaining = the async DRIVER, blocked on a carrier-layer surface:** one task per forwarding next-hop ticking `on_slot` + emitting over the forward sink + metering the SHARED per-carrier budget. It needs the carrier-selection layer to expose, per forwarding next-hop, **(i) which carrier reaches it + that carrier's cost/class** (for the per-link eligibility — the `CarrierSelectorSink` selects a carrier per-send and exposes no per-dyad "carrier facts" query), and **(ii) the shared per-carrier `BandwidthBudget`** (so padding meters into the same budget as real forwarding + cover, per the §6 "never starve forwarding"). **This is the SAME carrier-facts-per-link surface §7.3 carrier-diversity needs** ("which carrier is this build on") — design it once. Until that surface exists, the driver cannot be built without mis-layering (emitting blind to eligibility / metering a private budget — both gate-rejected on the scheduler).
