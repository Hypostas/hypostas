...
- **(a) Unauthenticated cover-drop (no new key, recommended for v1 — IN PROGRESS).** A padding cell is a random Outfox-shaped cell (correct fixed size + clear header) that authenticates *nowhere* at R+1 — it falls through R+1's existing **cover-drop** path (`on_data_outfox`'s final fall-through already silently drops a cell matching no forward/terminal/reverse leg) and is consumed, never forwarded. Delivers the §6 **threat property** in full (the observer can't distinguish padding from data; R+1 consumes it). Cost: R+1 does the normal peel-attempt work before dropping, and cannot *affirmatively* tell padding from a corrupt/attack cell (so no distinct padding accounting) — acceptable, since flow-correlation resistance doesn't need affirmative recognition. **No protocol change.** **BUILT (dyados PR #443):** the cell primitive `protocol_core::payload_onion::seal_outfox_padding_cell(size, circuit_id)` (indistinguishable from a forwarded DATA cell; consumed by the cover-drop — an integration test proves a padding cell fed to a live receiver drops silently). **Remaining (the driver):** collision-free id selection (the caller MUST pass an id NOT in its live demux tables — a blind random id can collide with a forwarding leg's `circuit_id_in`, and the forward path re-frames the random body WITHOUT authenticating, so a collision is *forwarded*, not dropped), the emit RATE (policy + energy class) + the `BandwidthBudget` step-down (COVER_TRAFFIC §5), off-by-default + carrier-tier gating, and the per-next-hop emit over the forward sink.
...
**Recommendation:** ship **(a)** for HYP-321 v1 (the threat property with zero protocol change), and track **(b)** as a follow-up optimization. **Decided + in progress:** the v1 cell primitive is built (PR #443); the *unauthenticated cover-drop* path needs no link key, so the cell IS a clean protocol-core primitive — it is the **authenticated `CMD_DROP` marker** (option b) that needs the absent link key, not the v1 cell. The driver (collision-free id + rate + budget) is the remaining v1 work. *(The Linear issue for this finding is pending the MCP connector reconnect — see the build ledger.)*
