...
**Status:** DESIGN **v6**, pre-review, 2026-08-06. v6 completes v5's demotion (the round-6 leg credited v5's *idea* as exactly right but found the correction incompletely propagated): §2b/§8/§4/changelog now uniformly say the dither **de-sharpens** the spike into a `~1.8–3×` shelf and does **NOT** remove the ≈`e^{−r}` mass; the R-dest-chain activity residual is a first-class §4 row (#16); the companion `.py`'s `lifetime_atom_detect` no longer hard-zeros the dithered case (the shelf block is the real computation) and its miscalibrated `N_target` call args are fixed. v5 supersedes v4 (review credited the mechanism AND the `{count,transient,lifetime} ⊥ activity` core as SOUND, but REFUTED the *headline*: the same-destination migration chain is an **activity** signal — conversation-presence — that I mis-filed as a "relationship / Phase-2" residual; and the membership bound was mis-cited). v5/v6 correct the *claims*, not the mechanism. Companion derivation: `circuit_spectral_bound_derivation.py`. Prereq: **HYP-331**. **The membership figure + Codex cross-vendor certification (HYP-533) are owed** — this doc iterates via the Claude MC-backed leg.
...
- **Destination migration chain (§6).** v3-review P2: a real conversation's successive carriers re-address the same peer while cover draws independently. *[superseded by v5: this is an ACTIVITY (conversation-presence) signal, NOT relationship, and NOT closed by mixing — closed only by recipient-anonymous delivery (Myco/YPIR). See §5.1 carve-out + §6 R-dest-chain.]*
...
Turnover inter-times are **`T = min(Exp(μ), MAX − U)`, `U ~ Uniform[0, D]`** — a memoryless exponential refresh, with a **dithered** hard cap: a circuit that would reach the deadline is instead torn over at a random point in `[MAX − D, MAX]`. Consequence: **there is no point *mass* at any single lifetime** (the v3 spike at exactly `MAX` is smeared over the dither window `D`), so the lifetime histogram carries no detectable *spike* and the spectral comb is broadened out. **But the ≈`e^{−r}` cap-hitter mass is NOT removed — it is reshaped into a smooth SHELF over `[MAX−D, MAX]`** (`circuit_spectral_bound_derivation.py` computes it: `~1.8–3×` excess over pure-exponential). So the dither **de-sharpens, it does not eliminate**: the residual is a smooth membership tell (§5.2), weaker than a spike but present; the exact detectability (dithered-vs-Poisson ROC) is **owed**. `D` is the knob (a fraction of `MAX`, e.g. 10%); the refresh rate `r = MAX/E[T]` is a separate cost/latency knob (HYP-171-tuned). **The bound this enables is stated structurally in §5, not as an info-theoretic Poisson claim** (the v3 over-claim).
...
## §4 Observable-match table (v5)

| # | Axis | Observable | Disposition (v5) |
|---|---|---|---|
| 1–12 | — | v1/v2 build/slot/sequence rows | MATCH / STRUCTURAL (#2 single-destination eligibility is the genuinely-structural distinguisher → §5) |
| 13 | sequence | refresh rate + spectrum + **lifetime histogram** | MATCH (activity) — dithered ⇒ no point mass/spike, no comb (§2b); residual is the smooth ≈`e^{−r}` shelf, role-independent → R-process (membership, §5.2) |
...
| **16** | **relationship→activity** | **successor-destination chain (R-dest-chain)** | **ACTIVITY RESIDUAL (§5.1 carve-out)** — occupied slot's successor re-addresses the same peer (`P(R=1)=1` vs cover `≈1/K`) ⇒ conversation-presence; NOT matched, NOT closed by mixing; closed only by recipient-anonymous delivery (Myco/YPIR, tracked) |
| 15 | sequence | host-restart count | STATED R-recovery — count collapse observable (§3b) |
| — | mobile | phone→host origination | timing MATCH via §3c covered channel; destination residual (R-2/R-mobile/R-dest-chain) |
...
- **R-dest-chain (ACTIVITY — the load-bearing one, §5.1 carve-out).** A real conversation's successive carriers re-address the same peer ⇒ an identity-blind conversation-presence signal. **NOT relationship, NOT closed by mixing.** **Candidate** close: recipient-anonymous delivery (Myco/YPIR) so every circuit terminates at the mailbox tier — **sufficiency owed**: it holds only if the GPA-observable mailbox ingest is universal/oblivious; if the mailbox is wire-observably sharded per recipient, the chain reappears *coarsened* at the mailbox layer (`P(same shard│same conversation)=1`). Tracked as a HYP-328-integration follow-up with that sufficiency check. Until then, the 522 guarantee is "no observable circuit BUILD," not "no observable conversation."
...
| `D` | dither width of the hard-cap teardown (`MAX−U`, `U∈[0,D]`) | **the #2 knob (de-sharpens the atom into a shelf — does NOT remove the ≈e^{−r} mass); ~10% of MAX** |
